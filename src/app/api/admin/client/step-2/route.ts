...
import { validateEntityType } from '@/lib/validations/client';
import { step2Schema } from '@/lib/validations/client-steps';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Validate input
    const validationResult = step2Schema.safeParse(stepData);
    ...
    // 3. Validate entity type (Specific to Step 2)
    ...
    // 4. Save via Service
    const result = await processGenericStep(sessionId, 2, { businessInfo: data });
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      data: session.session_data?.businessInfo || {},
    });
  } catch (error) {
    console.error('Step 2 GET API error:', error);
    return NextResponse.json(
      {
        error: 'Internal server error',
        details: error instanceof Error ? error.message : 'Unknown error',
