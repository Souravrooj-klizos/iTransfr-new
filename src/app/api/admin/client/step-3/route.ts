...
import { step3Schema } from '@/lib/validations/client-steps';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Validate input
    const validationResult = step3Schema.safeParse(stepData);
    ...
    // 3. Save via Service
    const result = await processGenericStep(sessionId, 3, { businessDetails: data });
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      data: session.session_data?.businessDetails || {},
    });
  } catch (error) {
